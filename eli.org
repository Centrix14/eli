...
1. While =word-readed-p= returns =nil=, read and perform data from stream.
2. Store character to buffer, if =store-char-p= returns =t= (in other
   words, if it's not a separator or quoting is active).
...
<<end-of-stream usage example>>:

#+begin_src lisp :tangle examples/parser/end-of-stream.lisp
...
In this example we print contents of specified file until EOF will be reached.

*** =word-readed-p=
This predicate returns =t= if word has been readed. That happens in when
all of the following statements are true.
1. Here is no quoting.
2. Here is EOF.
3. Here is a separator.
4. Here is some word in buffer.
...
*** =store-char-p=
This predicate returns =t= if char storing is necessary, and =nil=
otherwise. Char storing is necessary first || second statements are
true.
1. Here is a quoting.
2. Char is not a separator.
