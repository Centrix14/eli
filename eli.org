...
This is a standard method for all classes. =print-object= describes the
way to print an instance of some class to screen.
...
<<print-object usage example>>:
#+begin_src lisp :tangle examples/print-object.lisp
  (let ((test-parser
...
*** =pars-stream= method
This is a common method to pars LN expressions. It can pars strings
and files.

Important note: this method is *pure*, in other words it returns new
state of parser instead of changing it.
...
<<pars-stream usage example>>:
#+begin_src lisp :tangle examples/pars-stream.lisp
  (let ((test-parser
          (make-instance 'parser)))
